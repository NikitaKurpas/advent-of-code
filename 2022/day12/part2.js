...
const shortestPath = search(start);
...
function isLowestNode(node) {
    const [x, y] = node.split(",").map(Number);
    return grid[y][x] === 97 // 'a'
}
  ...
  const neighbors = [
    ...
    .filter((neighbor) => {
      ...
      return (
        ...
        (nodeCost > neighborCost ? nodeCost - neighborCost === 1 : true)
      );
    })
